...
import VersionDetailsCard from "../components/VersionDetailsCard2";
...
const PageProject = ({ classes, props }) => {
	...
	var version = (() => {
		for (const iterator of project.versions) {
			if (iterator[2] == versionId)
				return iterator
		}
	})()
	...
	var tabs = [
		...
		{ label: "Model", icon: <ProjectOwnerIcon fontSize="large" />, panel: <ProjectView /> },
		{ label: "Learning", icon: <TeamMessageIcon fontSize="large" />, panel: <VarialbleListView Label="Learning" ListArray={project.team} /> },
